...
export let renderDistX = 4;
export let renderDistY = 4;
export let renderDistZ = 4;
...
export function generateBlock(globalX, globalY, globalZ) {
...
export async function loadChunk(chunk, timeout = true) {
    ...
    let y = 0;
    let z = 0;
    while (totalIter < (Math.pow(chunk.size, 3))) {
        //console.log(x, y, z)
...
export async function chunkLoader() {
    ...
    for (let ch in surrounding) {
        ...
        if (!renderedChunks[`${chunkCoords[0]}/${chunkCoords[1]}/${chunkCoords[2]}`]) {
            //await request({type:"chunk", pos:{x:chunkCoords[0], y:chunkCoords[1], z:chunkCoords[2]}})
